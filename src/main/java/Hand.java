...
public abstract class Hand {
    ...
    public static String toString(Card[] cards) {
        ...
        Arrays.sort(cards);
        for(int j = 0; j < cards.length / 10 + 1; j++) {
            StringBuilder rk = new StringBuilder();
            StringBuilder st = new StringBuilder();
            for(int i = j * 10; i < (j+1) * 10 && i < cards.length; i++) {
                rk.append(cards[i].rankImg() + " ");
                st.append(cards[i].suitImg() + " ");
            }
            builder.append(rk.toString() + "\n" + st.toString());
            if(j < cards.length / 10) {
                builder.append("\n");
            }
        }
    ...
    /*
     * Deprecated version of toString that does not display Discord emote.
     * Converts the given hand into concatenation of their literal string representations
     * @param cards a hand of cards
     * @return a concatenation of the string representations of the given cards
     *
     * @deprecated
